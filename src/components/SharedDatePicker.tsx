...
export default function SharedDatePicker({
  ...
  const [todayMonth, setTodayMonth] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const cMonthStr = `${tYear}-${String(tMonth + 1).padStart(2, "0")}`;

    setTodayYear(tYear);
    setTodayMonth(tMonth);
    setTodayDateStr(tDateStr);
    setCurrentMonthStr(cMonthStr);
  ...
  const renderContent = () => {
    if (showYearSelectView) {
      return (
        <div className="fixed inset-0 z-[9999] bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 animate-backdrop-in">
          <div
            ref={containerRef}
            className="bg-[#162734] border-2 border-[#b8912f] rounded-xl p-4 shadow-2xl text-[#f2ece0] w-72 max-w-[90vw] animate-modal-in"
          >
            <div className="flex items-center justify-between mb-3 pb-2 border-b border-[#243b4d]">
              ...
              <button
                ...
                className="text-xs text-gray-400 hover:text-white font-mono p-1 rounded hover:bg-[#243b4d] transition"
              >
                Back
              </button>
            </div>
              ...
              {last6Years.map((y) => {
                ...
                return (
                  <button
                    ...
                    className={`py-2 text-xs font-mono font-bold rounded transition ${
                      isSelected
