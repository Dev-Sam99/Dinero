...
export default function CategoryPicker({
  ...
  return createPortal(
    <div
      onClick={onClose}
      className="fixed inset-0 z-[9999] bg-black/65 backdrop-blur-xs flex items-center justify-center p-4 animate-backdrop-in"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="bg-[#f2ece0] border-2 border-[#b8912f] rounded-xl p-4 shadow-2xl text-[#10202b] max-w-md w-full animate-modal-in"
      >
        <div className="flex items-center justify-between border-b border-[#d8ceba] pb-2 mb-3">
          ...
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="text-xs text-gray-500 hover:text-black font-mono font-bold p-1 rounded hover:bg-[#e4dbca] transition"
            >
              ✕ CLOSE
            </button>
          )}
        ...
        {showVehiclePicker && onVehicleAssigned ? (
          ...
          <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1">
            ...
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-lg text-xs font-semibold flex items-center justify-between pill-hover transition border ${
                selectedCategoryId === null
                  ? "bg-[#b8912f] text-white border-[#b8912f]"
                  : "bg-[#fbf8f3] text-gray-600 border-[#d8ceba]"
              }`}
            >
              ...
              return (
                <div key={loc.id} className="space-y-1.5">
                  <div className="flex items-center gap-2">
                    <span
                      className="w-2.5 h-2.5 rounded-full inline-block pill-hover"
                    ...
                    {locCategories.map((cat) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-3 py-2 rounded-lg text-xs font-medium flex items-center gap-2 pill-hover transition border ${
                            isSelected
                        ...
                        >
                          <div
                            className="w-6 h-6 rounded flex items-center justify-center text-white shrink-0 cat-icon-hover"
                            style={{ backgroundColor: loc.color }}
                          >
                            <IconComp className="w-3.5 h-3.5 transition-transform duration-180" />
                          </div>
